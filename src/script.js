...
const fog = new THREE.Fog("#141414", 1, 15);
scene.fog = fog;
...
const loadTexture = (path) => {
  return textureLoader.load(new URL(path, import.meta.url).href);
};
...
const bricksAmbientOcclusionTexture = loadTexture(
...
const grassAmbientOcclusionTexture = loadTexture(
...
const roof = new THREE.Mesh(
  new THREE.ConeGeometry(3.15, 1, 4),
...
bush1.position.set(2, 0.2, 3.2);

const bush2 = new THREE.Mesh(bushGeometry, bushMaterial);
bush2.scale.set(0.3, 0.3, 0.3);
bush2.position.set(-2, 0.2, 3.2);

const bush3 = new THREE.Mesh(bushGeometry, bushMaterial);
bush3.scale.set(0.5, 0.5, 0.5);
bush3.position.set(-4, 0.2, -3.2);

const bush4 = new THREE.Mesh(bushGeometry, bushMaterial);
bush4.scale.set(0.4, 0.4, 0.4);
bush4.position.set(4, 0.2, -2.2);
...
for (let i = 0; i < 50; i++) {
  ...
  const radius = 4 + Math.random() * 6.2;
  const x = Math.sin(angle) * radius;
  const z = Math.cos(angle) * radius;

  const grave1 = new THREE.Mesh(graveGeometry, graveMaterial);
  grave1.position.set(x, 0.3, z);
  grave1.rotation.y = (Math.random() - 0.5) * 0.4;
  grave1.rotation.z = (Math.random() - 0.5) * 0.4;

  //shadow
  grave1.castShadow = true;
...
// Directional light
const moonLight = new THREE.DirectionalLight("#b9d5ff", 0.01);
...
const doorLight = new THREE.PointLight("#ffd58b", 2, 7);
doorLight.position.set(0, 2.2, 2.7);
house.add(doorLight);

//& GHOSTS

const ghost1 = new THREE.PointLight("blue", 6, 3);
scene.add(ghost1);
const ghost2 = new THREE.PointLight("pink", 6, 3);
scene.add(ghost1);
